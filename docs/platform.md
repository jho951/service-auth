...
implementation libs.platform.security.auth.bridge.starter
...
- auth-service의 security 의존은 `platform-runtime-bom`, `platform-security-starter`, `platform-security-auth-bridge-starter`, `platform-security-ratelimit-bridge-starter`로 제한합니다.
- security audit event를 governance audit-log로 연결할 때만 `platform-integrations`의 `platform-security-governance-bridge`를 사용합니다.
- raw auth/rate-limit 연동은 service-owned wrapper가 아니라 공식 bridge starter가 담당합니다.
...
auth-service 도메인 코드는 governance adapter/core/engine 구현 타입을 직접 소비하지 않습니다.
...
- `AuthAuditLogService`만 `AuditLogger`를 주입받습니다.
- `AuditSink`, `AuditLogger`, `AuditEvent`를 직접 쓰는 compile surface는 `platform-governance-starter`가 아니라 `audit-log-api`가 제공합니다.
- 로그인, 로그아웃, internal account 변경 시점은 auth-service 도메인 코드에서 명시합니다.
- `AuthAuditLogService`는 도메인 이벤트를 `AuditEvent`로 변환하는 adapter 역할만 합니다.
