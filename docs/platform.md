...
Auth-server는 `platform-security`, `platform-governance`, `platform-integrations`를 공통 경계로만 사용합니다.
...
| `platform-integrations` | `1.0.0` |

`platform-resource`는 파일/리소스 저장 runtime이므로 Auth-server에는 붙이지 않습니다. Auth-server에 필요한 integration은 security audit event를 governance audit으로 연결하는 `platform-security-governance-bridge`뿐입니다.

## Security

Auth-server는 token/session issuer가 주 역할이므로 `platform-security-starter`와 `ISSUER` preset을 사용합니다.
...
`platform-security-governance-bridge`는 platform-security deny/admin/internal audit event를 governance audit recorder로 연결할 때만 사용합니다. Auth-server는 두 플랫폼을 모두 사용하고 보안 경계 이벤트도 감사 대상이므로 이 bridge를 붙입니다.
