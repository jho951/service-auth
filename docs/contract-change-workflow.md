...
이 문서는 `auth-service` 구현 코드를 수정한 뒤 계약 변경을 어떻게 처리할지 정리한다. 기준 계약의 원본은 `https://github.com/jho951/service-contract`이고, 이 repo는 `contract.lock.yml`로 특정 contract commit을 따른다.
...
`service-contract`, `docs/openapi/auth-service.yml`, `contract.lock.yml`을 건드리지 않는다.
...
git commit -m "Update auth-service implementation"
...
git commit -m "Update auth-service contract"
...
OpenAPI가 바뀌면 아래 두 파일을 같은 내용으로 맞춘다.

```bash
cp /Users/jhons/Downloads/BE/contract/service-contract/artifacts/openapi/auth-service.upstream.v1.yaml \
  docs/openapi/auth-service.yml
```

- contract 원본: `artifacts/openapi/auth-service.upstream.v1.yaml`
- service local copy: `docs/openapi/auth-service.yml`
...
git commit -m "Align auth-service implementation with contract"
git push origin <branch>
```
...
| 변경 내용 | 올릴 위치 |
| --- | --- |
| API path, request, response, status, auth 변경 | `service-contract/artifacts/openapi`, `service-contract/repositories/auth-service` |
| 공통 header, error, security, env, CI/CD 규칙 변경 | `service-contract/shared` |
| 운영 port, health, ready, compose, monitoring target 변경 | `service-contract/repositories/auth-service/ops.md` 또는 관련 운영 문서 |
